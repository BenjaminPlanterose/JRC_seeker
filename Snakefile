...
rule meth_info:
	...
	shell:
		...
		#biscuit mergecg {input.ref_genome} temp.bed | cut -f1-5 > {output}
...
rule binpolish:
	...
	shell:
		"""
		Rscript scripts/binpolish.R -s {input.segments} -l {input.labels} -b {params.blacklist} -m {params.mappability} -y {params.segment_min_sz} -i {params.binpolishflank} -o {output.bp} -r {output.reg}
		"""
